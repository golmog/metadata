...
class ModuleJavCensored(PluginModuleBase):
    ...
    def plugin_load(self):
        ...
        try:
            ...
            logger.info(f"[{self.name}] Dedicated Metadata DB initialized perfectly at metadata_av.db")
            self.web_list_model = ModelAvMetadata
        except Exception as e_db_create:
            logger.error(f"[{self.name}] Failed to create Dedicated Metadata DB: {e_db_create}")
            logger.error(traceback.format_exc())
    ...
    def process_ajax(self, sub, req):
        try:
            ...
            logger.error(f"[{self.name}] process_ajax 통신 예외 발생: {e}")
            logger.error(traceback.format_exc())
    ...
    def process_command(self, command, arg1, arg2, arg3, req):
        try:
            ...
            if command == "db_crop_save":
                ...
                ret['ret'] = 'success' if success else 'error'
                return jsonify(ret)
                ...
                if not raw_json_str:
                    return jsonify({'ret': 'error', 'msg': '수정할 JSON 데이터가 서버로 전달되지 않았습니다.'})
                ...
                try:
                    ...
                    if success:
                        ...
                        return jsonify({'ret': 'error', 'msg': 'DB 업데이트 실패 (해당 품번을 찾을 수 없음)'})
                ...
                ret['ret'] = 'success' if success else 'error'
                ret['msg'] = f"모든 메타데이터 DB가 초기화되었습니다. ({count}건 삭제됨)" if success else "초기화 실패"
                return jsonify(ret)
                ...
                ret['ret'] = 'success' if success else 'error'
                ret['msg'] = "DB 최적화(VACUUM)가 완료되었습니다." if success else "최적화 실패"
                return jsonify(ret)
                ...
                site_char_map = {'B': 'javbus', 'D': 'dmm', 'T': 'jav321', 'M': 'mgstage', 'J': 'javdb'}
                site = site_char_map.get(code[1], 'dmm') if len(code) > 1 else 'dmm'
                
                fresh_data = self.info2(code, site, keyword=None, fp_meta_mode=False, skip_trans=True, is_validating=False)
                if fresh_data:
                    cached_json['thumb'] = fresh_data.get('thumb', [])
    ...
    def _reconcile_official_results(self, all_results):
        """
        DMM/MGStage가 접두사 차이로 99점을 받고 JavDB/Javbus/Jav321이 접두사 없는 품번으로 100점을 받은 경우,
        순수 레이블/넘버 및 엄격한 제목 유사도(65% 이상/상호포함)를 대조하여 DMM/MGStage 페널티 제거 (100점 승격)
        """
        if not all_results:
            return all_results
        ...
        if not official_items or not secondary_items:
            return all_results
        ...
        for off_item in official_items:
            ...
            for sec_item in secondary_items:
                ...
                # 2. 순수 레이블 영문 & 넘버 완벽 일치 검사 (예: SPSC == SPSC and 66 == 66)
                if off_label == sec_label and off_num == sec_num:
                    ...
                    # 3. 제목 유사도 엄격 검증 (유사도 65% 이상이거나 5글자 이상 핵심 문장 상호 포함)
                    ...
                    is_title_matched = is_contained or (similarity >= 0.65)

                    if is_title_matched:
                        logger.info(
                            f"[{self.name}] ★ 공식 사이트 품번 승격: "
                            f"[{off_item.get('site_key', '').upper()}] '{off_ui}'(원점수: 99 ➔ 100) ➔ "
                            f"[{sec_item.get('site_key', '').upper()}] '{sec_ui}'(100점)와 동일 작품 검증 완료 (유사도: {similarity:.2f}). "
                            f"접두사 페널티를 제거하여 공식 메타를 100점으로 승격합니다."
                        )
    ...
    def search(self, keyword, manual=False):
            ...
            if site_results:
                ...
                if not manual:
                    for item in site_results:
                        if site_key in ['dmm', 'mgstage'] and item.get('original_score', 0) >= 100:
                            logger.debug(f"[{self.name}] Early Exit: 공식 사이트 '{site_key}'에서 100점 매칭 확정. 검색 중단: {keyword}")
        ...
        # 공식 사이트(DMM/MGS)와 타 사이트 간의 품번 접두사 차이 검증 및 100점 승격 처리
            ...
            for i, item_in_sorted_list in enumerate(all_results_sorted):
                ...
                if i == 0:
                    ...
                    if raw_score >= prev_score:
                        item_in_sorted_list['score'] = max(0, prev_score - 1)
                    else:
                        item_in_sorted_list['score'] = raw_score

                if manual:
                    code_to_bypass = item_in_sorted_list['code']
                    try:
                        self.keyword_cache.set(f"BYPASS_{code_to_bypass}", "1")
                    except AttributeError:
    ...
    def info(self, code, keyword=None, fp_meta_mode=False, skip_trans=False):
        ...
        # --- DB에서 데이터 가져오기 & 빈 데이터 보완(Enrichment) ---
        use_db = P.ModelSetting.get_bool(f"{self.name}_db_use")
            ...
            if cached_json:
                ...
                if not is_db_untranslated:
                    ...
                    if needs_enrichment:
                        ...
                        fresh_data = self.info2(code, site, keyword, fp_meta_mode=False, skip_trans=True, is_validating=False)
                        if fresh_data:
